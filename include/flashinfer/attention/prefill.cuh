...
template <bool produce_v, uint32_t NUM_MMA_KVQ_UNRLD, uint32_t UNRLkvq, uint32_t UNRLz, uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_D,
          uint32_t NUM_MMA_KV, SwizzleMode swizzle_mode, typename DType, typename IdType, bool isLoad>
...
#pragma unroll
      for (uint32_t mma_d = 0; mma_d < NUM_MMA_D; ++mma_d) {
        ...
        uint32_t o_smem_offset_w = o_smem->template get_permuted_offset<channel_size_128b_out>(
            (warp_idx_x * NUM_MMA_Q + mma_q) * 16 + real_lane_idx % 16,
            real_lane_idx / 16 / 2 + mma_d * 2);
        
        auto* dest = (unsigned char*)(o_smem->base + o_smem_offset_w) \
          + sizeof(o_frag_f16) * (real_lane_idx / 16 % 2);
    ...
    const uint32_t row_ = warp_idx_x * num_rows_per_warp
                        + real_lane_idx / num_threads_per_row;
...
#pragma unroll
    for (uint32_t write_idx = 0; write_idx < num_writes; ++write_idx) {
      ...
      uint32_t offset = o_smem->template get_permuted_offset<channel_size_128b_out>(
        row, col / num_elems_per_128b<DTypeO>());

      // read from shared memory
      DTypeOv o_frag_f16 = *(reinterpret_cast<DTypeOv*>( (DTypeO*)(o_smem->base + offset) + (col % num_elems_per_128b<DTypeO>())));
...
#pragma unroll
    for (uint32_t i = 0;
           i < NUM_MMA_KV * (swizzle_mode_kv == SwizzleMode::k128B ? 4 : 2) / NUM_WARPS_Q; ++i) {
        uint32_t page_iter = (packed_page_iter_base + (16 * NUM_WARPS_KV * NUM_MMA_KV) + warp_idx * kv_frag_rows + lane_idx / kv_frag_cols + kv_frag_rows * NUM_WARPS_Q * NUM_WARPS_KV * i)/paged_kv.page_size;
...
#pragma unroll
      ...
      page_produce_kv<false, NUM_MMA_KVQ_UNRLD, UNRLkvq, UNRLz, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D, NUM_MMA_KV>(
          ...
          kv_offset, chunk_size, load_vals[0], std::true_type{});
      ...
      page_produce_kv<true, NUM_MMA_KVQ_UNRLD, UNRLkvq, UNRLz, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D, NUM_MMA_KV>(
          ...
          kv_offset, chunk_size, load_vals[1], std::true_type{});
