...
template <typename DType> struct mfma_m16n16k16_f32
{
    ...
    static __device__ __forceinline__ c_fragment_type run(ab_fragment_type, ab_fragment_type, c_fragment_type)
    {
        return c_fragment_type{};
    }
};

template <> struct mfma_m16n16k16_f32<__half>
{
    using ab_fragment_type = B16x4_t;
    using c_fragment_type = B32x4_t;

    static __device__ __forceinline__ c_fragment_type run(ab_fragment_type a, ab_fragment_type b, c_fragment_type c)
    {
        return __builtin_amdgcn_mfma_f32_16x16x16f16(a, b, c, 0, 0, 0);
    }
};

template <> struct mfma_m16n16k16_f32<__hip_bfloat16>
{
    using ab_fragment_type = B16x4_t;
    using c_fragment_type = B32x4_t;
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV> __device__ __forceinline__ uint32_t get_warp_idx_q()
{
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV> __device__ __forceinline__ uint32_t get_warp_idx_kv()
{
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV> __device__ __forceinline__ uint32_t get_warp_idx()
{
    return get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>() * NUM_WARPS_Q + get_warp_idx_q<NUM_WARPS_Q, NUM_WARPS_KV>();
}
...
template <typename T>
__device__ __forceinline__ void q_frag_apply_llama_rope(T *x_first_half, T *x_second_half, const float *rope_freq,
...
__device__ __forceinline__ void q_frag_apply_llama_rope_with_pos(T *x_first_half, T *x_second_half,
                                                                 ...
                                                                 const uint_fastdiv group_size, const IdType *q_offset)
{
 ...
 */
template <SharedMemFillMode fill_mode, uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_D,
          uint32_t NUM_MMA_KV, SwizzleMode swizzle_mode, typename T>
__device__ __forceinline__ void produce_kv(smem_t<swizzle_mode> smem, uint32_t *smem_offset, T **gptr,
                                                ...
                                                const size_t *kv_offset, const uint32_t kv_len)
{
    ...
    if constexpr (swizzle_mode == SwizzleMode::k128B)
    {
        ...
        static_assert(NUM_MMA_KV * 4 % NUM_WARPS_Q == 0);
        // printf("\n-----%d,------\n", NUM_WARPS_Q);
        constexpr uint32_t UNRLkvq_ATMP      = 2;
	    ...
	    kv_idx = kv_idx_ORIG;
            ...
            for (uint32_t i_ = 0; i_ < UNRLkvq; ++i_)
            {
                ...
                for (uint32_t j = 0; j < NUM_MMA_D / (8 / sizeof(DType)); ++j)
                {
                    ...
                    if ((fill_mode == SharedMemFillMode::kFillZero) || (kv_idx < kv_len))
                    {
                                                ...
                                                std::bool_constant<isLoad> = {})
{
        ...
        if constexpr (!isLoad)
            for (uint32_t i = 0; i < NUM_MMA_KVQ_UNRLD; ++i)
            {
                ...
                for (uint32_t i_ = 0; i_ < UNRLkvq; ++i_)
                {
                    ...
                    {
                        b128_t *smem_ptr = smem.base + *smem_offset;
                        *((uint4 *)smem_ptr) = load_vals[i][i_][j];
                        *smem_offset = smem.template advance_offset_by_column<8>(*smem_offset, j);
                    }
                    *smem_offset =
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < NUM_MMA_KV * 2 / NUM_WARPS_Q; ++i)
        {
            ...
            // smem.load_128b_async<fill_mode>(*smem_offset, gptr, kv_idx < kv_len);
            // hipFIXED
            smem.template load_128b_async<fill_mode>(*smem_offset, gptr, kv_idx < kv_len); // predicated loads are slow
            kv_idx += num_warps * 8; // can we precalc inbounds?
            *smem_offset = smem.template advance_offset_by_row<num_warps * 8, channel_size_128b_kv>(*smem_offset);
        }
        *smem_offset -= NUM_WARPS_KV * NUM_MMA_KV * 16 * channel_size_128b_kv;
    }
...
__device__ __forceinline__ void init_rope_freq(float (*rope_freq)[4], const float log2_rope_rcp_scale,
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, SwizzleMode swizzle_mode,
          typename DTypeQ>
__device__ __forceinline__ void load_q_global_smem(uint32_t packed_offset, const uint32_t qo_upper_bound,
                                                   DTypeQ *q_ptr_base, const uint32_t q_ptr_thread_offset, const uint32_t q_stride_n,
                                                   ...
                                                   smem_t<swizzle_mode> *q_smem)
{
    ...
    if (get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>() == 0)
    {
        ...
        ck_tile::uint16x8_t load_vals[NUM_MMA_Q][4][NUM_MMA_D / 4] = {};
        #pragma unroll
        for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q)
        {
            #pragma unroll
            for (uint32_t j = 0; j < 4; ++j)
            {
                ...
                uint32_t q_ptrOffset = q_ptr_thread_offset + q * q_stride_n + r * q_stride_h;
                uint16_t * ck_q_ptr_base = reinterpret_cast<uint16_t*>(q_ptr_base);
                #pragma unroll
                for (uint32_t mma_do = 0; mma_do < NUM_MMA_D / 4; ++mma_do)
                {
                    using index_t = ck_tile::index_t;
                    auto buff = ck_tile::amd_buffer_load_invalid_element_return_zero<uint16_t, (index_t)8>(ck_q_ptr_base,
                                                                                                           q_ptrOffset,
                                                                                                          (q_idx < qo_upper_bound),
                                                                                                           0x3FFFFFFF);
                    load_vals[mma_q][j][mma_do] = buff.template get_as<ck_tile::uint16x8_t>().get(0);

                    q_ptrOffset += 8 * num_elems_per_128b<DTypeQ>();
                }
            }
        }
        #pragma unroll
        for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q)
        {
            ...
            for (uint32_t j = 0; j < 4; ++j)
            {
                ...
                for (uint32_t mma_do = 0; mma_do < NUM_MMA_D / 4; ++mma_do)
                {
                    ck_tile::uint16x8_t *smem_ptr = reinterpret_cast<ck_tile::uint16x8_t *>(q_smem->base + q_smem_offset_w);
                    *(smem_ptr) = load_vals[mma_q][j][mma_do];
                    q_smem_offset_w = q_smem->template advance_offset_by_column<8>(q_smem_offset_w, mma_do);
                }
                q_smem_offset_w = q_smem->template advance_offset_by_row<4, channel_size_128b_q>(q_smem_offset_w) -
                                  (NUM_MMA_D / 4) * 8;
            }
        }
    }
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, SwizzleMode swizzle_mode,
          typename DTypeQ>
__device__ __forceinline__ void q_smem_inplace_apply_rotary(const uint32_t q_packed_idx, const uint32_t qo_len,
                                                            const uint32_t kv_len, const uint_fastdiv group_size,
                                                            smem_t<swizzle_mode> *q_smem, uint32_t *q_smem_offset_r,
                                                            float (*rope_freq)[4])
{
    if (get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>() == 0)
    {
...
__device__ __forceinline__ void q_smem_inplace_transform(const typename AttentionVariant::ParamsT &params,
                                                         AttentionVariant variant, smem_t<swizzle_mode> *q_smem)
{
...
__device__ __forceinline__ void k_smem_inplace_apply_rotary(const uint32_t kv_idx_base, smem_t<swizzle_mode> *k_smem,
                                                            uint32_t *k_smem_offset_r, float (*rope_freq)[4])
{
    ...
    if constexpr (NUM_MMA_D == 4 && NUM_WARPS_Q == 4)
    {
        ...
        *k_smem_offset_r = (*k_smem_offset_r ^ (0x2 * (warp_idx % 2))) + (warp_idx / 2) * 16 * channel_size_128b_kv;
#pragma unroll
        for (uint32_t i = 0; i < NUM_MMA_KV / 2; ++i)
        {
            ...
            k_frag_apply_llama_rope<DTypeKV>((DTypeKV *)k_frag_local[0], (DTypeKV *)k_frag_local[1], rope_freq[mma_di],
                                             kv_idx);
        ...
        *k_smem_offset_r =
            (*k_smem_offset_r ^ (0x2 * (warp_idx % 2))) - ((warp_idx / 2) + NUM_MMA_KV) * 16 * channel_size_128b_kv;
    }
    else
    {
        ...
        for (uint32_t i = 0; i < NUM_MMA_KV; ++i)
        {
            ...
            for (uint32_t j = 0; j < NUM_MMA_D / (2 * NUM_WARPS_Q); ++j)
            {
                ...
                k_smem_offset_r_first_half =
                    k_smem->template advance_offset_by_column<2 * NUM_WARPS_Q>(k_smem_offset_r_first_half, mma_di);
            }
            *k_smem_offset_r += 16 * channel_size_128b_kv;
            kv_idx += 16;
        }
        *k_smem_offset_r = (*k_smem_offset_r ^ (0x2 * warp_idx_x)) - NUM_MMA_KV * 16 * channel_size_128b_kv;
    }
}
...
template <MaskMode MASK_MODE, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, uint32_t NUM_MMA_KV, typename AttentionVariant,
          typename DTypeQKAccum>
__device__ __forceinline__ void logits_mask(const typename AttentionVariant::ParamsT &params, AttentionVariant variant,
                                            const uint32_t batch_idx, const uint32_t qo_packed_idx_base,
                                            const uint32_t kv_idx_base, const uint32_t qo_len, const uint32_t kv_len,
    ...
    for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q)
    {
        ...
        for (uint32_t mma_kv = 0; mma_kv < NUM_MMA_KV; ++mma_kv)
        {
            ...
            for (uint32_t reg_id = 0; reg_id < 4; ++reg_id)
            {
                ...
                {
                    mask = !(kv_idx >= chunk_end);
                }
                mask = mask && variant.LogitsMask(params, batch_idx, q_idx, kv_idx, qo_head_idx, kv_head_idx);
...
template <uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, uint32_t NUM_MMA_KV, typename DTypeQKAccum, typename AttentionVariant>
__device__ __forceinline__ void update_mdo_states(AttentionVariant variant, DTypeQKAccum (*s_frag)[NUM_MMA_KV][4],
                                                  float (*o_frag)[NUM_MMA_D][4], DTypeQKAccum (*m)[1], float (*d)[1])
{
    if constexpr (variant.use_softmax)
    {
        if constexpr (std::is_same_v<DTypeQKAccum, float>)
        ...
        else if constexpr (std::is_same_v<DTypeQKAccum, half>)
        {
            ...
            for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q)
            {
                ...
                for (uint32_t j = 0; j < 2; ++j)
                {
                    ...
                    {
#if defined(__HIPCC__) || (defined(__clang__) && defined(__HIP__)) || defined(__HIPCC_RTC__)
                        __hip_bfloat162 temp_bf162 =
                            __hmax2(convert_half2_to_bfloat162(*(half2 *)&s_frag[mma_q][mma_kv][j * 2]),
...
#if defined(__HIPCC__) || (defined(__clang__) && defined(__HIP__)) || defined(__HIPCC_RTC__)
                __hip_bfloat162 temp_bf162 =
                    __hmax2(convert_half2_to_bfloat162(*(half2 *)&m[mma_q]),
        ...
        for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q)
        {
            ...
            for (uint32_t mma_kv = 0; mma_kv < NUM_MMA_KV; ++mma_kv)
            {
                ...
                else
                {
                    ...
                    {
                        s_frag_f16[mma_q][mma_kv][i] = float2T_unsafe<DTypeQ>(s_frag[mma_q][mma_kv][i]);
                    }
                }
            }
        }
...
#pragma unroll
            for (uint32_t j = 0; j < 1; ++j)
            {
                d_rcp[mma_q][j] = (m[mma_q][j] != DTypeQKAccum(-math::inf)) ? __builtin_amdgcn_rcpf(d[mma_q][j]) : 0.f;
            }
        }
...
#pragma unroll
                for (uint32_t reg_id = 0; reg_id < 4; ++reg_id)
                {
                    o_frag[mma_q][mma_d][reg_id] = o_frag[mma_q][mma_d][reg_id] * d_rcp[mma_q][0];
                }
            }
        }
    }
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, typename DTypeQKAccum,
          typename AttentionVariant>
__device__ __forceinline__ void threadblock_sync_mdo_states(AttentionVariant variant, float (*o_frag)[NUM_MMA_D][4],
                                                            float *smem_workspace, DTypeQKAccum (*m)[1], float (*d)[1],
                                                            const uint32_t warp_idx, const uint32_t lane_idx)
{
    ...
    {
        float2 *smem_md =
            (float2 *)(smem_workspace + NUM_MMA_Q * NUM_MMA_D * NUM_WARPS_Q * NUM_WARPS_KV * WARP_SIZE * 4);
...
#pragma unroll
            for (uint32_t mma_d = 0; mma_d < NUM_MMA_D; ++mma_d)
            {
                vec_t<float, 4>::memcpy(
                    smem_workspace + (((warp_idx * NUM_MMA_Q + mma_q) * NUM_MMA_D + mma_d) * WARP_SIZE + lane_idx) * 4,
                    o_frag[mma_q][mma_d]);
            }
        }
...
#pragma unroll
                    ...
                    {
                        float2 md =
                            smem_md[(((i * NUM_WARPS_Q + get_warp_idx_q<NUM_WARPS_Q, NUM_WARPS_KV>()) * NUM_MMA_Q +
                                      mma_q) *
                                         1 +
                                     j) *
                                        WARP_SIZE +
                        ...
                        d_new = d_prev * __builtin_amdgcn_exp2f(m_prev - m_new) +
                                md.y * __builtin_amdgcn_exp2f(md.x - m_new);
                    }
...
#pragma unroll
                    for (uint32_t i = 0; i < NUM_WARPS_KV; ++i)
                    {
                        ...
                        oi.load(
                            smem_workspace +
                            ((((i * NUM_WARPS_Q + get_warp_idx_q<NUM_WARPS_Q, NUM_WARPS_KV>()) * NUM_MMA_Q + mma_q) *
                                  NUM_MMA_D +
                              mma_d) *
                                 WARP_SIZE +
                             lane_idx) *
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D, SwizzleMode swizzle_mode,
          typename DTypeO>
__device__ __forceinline__ void write_o_reg_gmem(float (*o_frag)[NUM_MMA_D][4], smem_t<swizzle_mode> *o_smem,
...
#pragma unroll
            for (uint32_t mma_d = 0; mma_d < NUM_MMA_D; ++mma_d)
            {
                ...
                uint32_t o_smem_offset_w = o_smem->template get_permuted_offset<channel_size_128b_out>(
                    (warp_idx_x * NUM_MMA_Q + mma_q) * 16 + real_lane_idx % 16, real_lane_idx / 16 / 2 + mma_d * 2);

                auto *dest =
                    (unsigned char *)(o_smem->base + o_smem_offset_w) + sizeof(o_frag_f16) * (real_lane_idx / 16 % 2);
                memcpy(dest, o_frag_f16, sizeof(o_frag_f16));
            }
        }

        constexpr uint32_t NUM_THREADS = NUM_WARPS_Q * WARP_SIZE;
        constexpr uint32_t num_total_elems = (NUM_WARPS_Q * NUM_MMA_Q * 16) * head_dim;
        constexpr uint32_t elems_per_thread = num_total_elems / NUM_THREADS;

        constexpr uint32_t elems_per_write = 8; // TODO: use larger vector size
        constexpr uint32_t num_writes = elems_per_thread / elems_per_write;
        static_assert(num_elems_per_128b<DTypeO>() % elems_per_write == 0);

        constexpr uint32_t num_threads_per_row = head_dim / elems_per_write;
        constexpr uint32_t num_rows_per_warp = WARP_SIZE / num_threads_per_row;
        static_assert(num_threads_per_row <= WARP_SIZE);

        __syncthreads();
        using DTypeOv = __attribute__((__vector_size__(elems_per_write / 2 * sizeof(float)))) float;
        const uint32_t row_ = warp_idx_x * num_rows_per_warp + real_lane_idx / num_threads_per_row;
        const uint32_t col = (real_lane_idx % num_threads_per_row) * elems_per_write;
#pragma unroll
        for (uint32_t write_idx = 0; write_idx < num_writes; ++write_idx)
        {
            // calculate (row, col) in output tile
            const uint32_t row = row_ + write_idx * (NUM_WARPS_Q * num_rows_per_warp);

            uint32_t offset =
                o_smem->template get_permuted_offset<channel_size_128b_out>(row, col / num_elems_per_128b<DTypeO>());

            // read from shared memory
            DTypeOv o_frag_f16 = *(
                reinterpret_cast<DTypeOv *>((DTypeO *)(o_smem->base + offset) + (col % num_elems_per_128b<DTypeO>())));

            uint32_t q, r;
            group_size.divmod(o_packed_idx_base + row, q, r);

            // skip writing if out-of-bound
            const uint32_t o_idx = q;
            if (qo_upper_bound <= o_idx)
            {
                break;
            }
            ...
            *(reinterpret_cast<DTypeOv *>(o_ptr + col)) = o_frag_f16;
        }
    }
}
...
template <MaskMode MASK_MODE, flashinfer::PosEncodingMode POS_ENCODING_MODE, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D,
          ...
          typename AttentionVariant>
__global__ __launch_bounds__(NUM_WARPS_Q *NUM_WARPS_KV *WARP_SIZE) void SinglePrefillWithKVCacheKernel(
    const uint_fastdiv group_size,
#if defined(__HIPCC__) || (defined(__clang__) && defined(__HIP__)) || defined(__HIPCC_RTC__)
    ...
    const __grid_constant__ typename AttentionVariant::ParamsT params)
{
    ...
    else
    {
        ...
        const uint32_t lane_idx = threadIdx.x % CUDA_WARP_SIZE, real_lane_idx = threadIdx.x,
        ...
        const tensor_info_t qkv_info(qo_len, kv_len, num_qo_heads, num_kv_heads, q_stride_n, q_stride_h, kv_stride_n,
                                     kv_stride_h, /*head_dim=*/NUM_MMA_D * 16);
        ...
        uint32_t q_ptr_thread_offset = qkv_info.get_q_elem_offset(0, kv_head_idx * group_size, (lane_idx % 8) * num_elems_per_128b<DTypeQ>());
        ...
        uint32_t q_smem_offset_r = qo_smem.get_permuted_offset<channel_size_128b_q>(
            get_warp_idx_q<NUM_WARPS_Q, NUM_WARPS_KV>() * NUM_MMA_Q * 16 + lane_idx % 16, lane_idx / 16);

        load_q_global_smem<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D>(
            qo_packed_idx_base, qo_len, q, q_ptr_thread_offset, q_stride_n, q_stride_h, group_size, &qo_smem);
        ...
        if constexpr (POS_ENCODING_MODE == PosEncodingMode::kRoPELlama)
        {
            q_smem_inplace_apply_rotary<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D, swizzle_mode_q, DTypeQ>(
        ...
        const uint32_t num_iterations = ceil_div(
            MASK_MODE == MaskMode::kCausal
                ? min(chunk_size,
                      sub_if_greater_or_zero(kv_len - qo_len + ((bx + 1) * num_rows_per_cta) / group_size, chunk_start))
                : chunk_size,
            16 * NUM_WARPS_KV * NUM_MMA_KV);

        const uint32_t window_iteration =
            ceil_div(sub_if_greater_or_zero(kv_len + (bx + 1) * num_rows_per_cta, qo_len + window_left + chunk_start),
                     (16 * NUM_WARPS_KV * NUM_MMA_KV));

        const uint32_t mask_iteration =
            (MASK_MODE == MaskMode::kCausal
                 ? min(chunk_size,
                       sub_if_greater_or_zero(kv_len + (bx * num_rows_per_cta) / group_size - qo_len, chunk_start))
                 : chunk_size) /
            (16 * NUM_WARPS_KV * NUM_MMA_KV);

        DTypeKV *k_ptr =
            k + qkv_info.get_kv_elem_offset(chunk_start + warp_idx * kv_frag_rows + lane_idx / kv_frag_cols,
                                            kv_head_idx, (lane_idx % kv_frag_cols) * num_elems_per_128b<DTypeKV>());
        DTypeKV *v_ptr =
            v + qkv_info.get_kv_elem_offset(chunk_start + warp_idx * kv_frag_rows + lane_idx / kv_frag_cols,
                                            kv_head_idx, (lane_idx % kv_frag_cols) * num_elems_per_128b<DTypeKV>());
        ...
        for (uint32_t iter = 0; iter < num_iterations; ++iter)
        {
            ...
            if constexpr (POS_ENCODING_MODE == PosEncodingMode::kRoPELlama)
            {
                k_smem_inplace_apply_rotary<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D, NUM_MMA_KV, swizzle_mode_kv, DTypeKV>(
                    chunk_start + iter * 16 * NUM_WARPS_KV * NUM_MMA_KV, &k_smem, &k_smem_offset_r, rope_freq);
                block.sync();
            }
            ...
            if (MASK_MODE == MaskMode::kCustom || (iter >= mask_iteration || iter < window_iteration))
            {
                logits_mask<MASK_MODE, NUM_MMA_Q, NUM_MMA_D, NUM_MMA_KV>(
                    params, variant, /*batch_idx=*/0, qo_packed_idx_base,
                    chunk_start +
                        (iter * NUM_WARPS_KV + get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>()) * NUM_MMA_KV * 16,
            ...
            block.sync();
            produce_kv<SharedMemFillMode::kNoFill, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D, NUM_MMA_KV>(
                k_smem, &kv_smem_offset_w, &k_ptr, kv_stride_n, (iter + 1) * 16 * NUM_WARPS_KV * NUM_MMA_KV,
                chunk_size);
            cp_async::commit_group();
            cp_async::wait_group<1>();
            block.sync();
            ...
            block.sync();
            produce_kv<SharedMemFillMode::kFillZero, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D, NUM_MMA_KV>(
                v_smem, &kv_smem_offset_w, &v_ptr, kv_stride_n, (iter + 1) * 16 * NUM_WARPS_KV * NUM_MMA_KV,
                chunk_size);
            cp_async::commit_group();
        }
        cp_async::wait_group<0>();
...
gpuError_t SinglePrefillWithKVCacheDispatched(typename AttentionVariant::ParamsT params,
                                              typename AttentionVariant::DTypeO *tmp, gpuStream_t stream)
{
    using DTypeQ = typename AttentionVariant::DTypeQ;
    using DTypeKV = typename AttentionVariant::DTypeKV;
    using DTypeO = typename AttentionVariant::DTypeO;
    ...
    DISPATCH_CTA_TILE_Q(cta_tile_q, CTA_TILE_Q, {
        ...
        using DTypeQKAccum =
            typename std::conditional<ALLOW_FP16_QK_REDUCTION && std::is_same_v<DTypeQ, half>, half, float>::type;
        ...
        DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem, max_num_mma_kv_reg), NUM_MMA_KV, {
            ...
            else
            {
                ...
                constexpr uint32_t num_rows_per_cta = NUM_MMA_Q * NUM_WARPS_Q * 16;
                auto kernel =
                    SinglePrefillWithKVCacheKernel<MASK_MODE, POS_ENCODING_MODE, NUM_MMA_Q, NUM_MMA_D, NUM_MMA_KV,
                ...
                uint32_t smem_size =
                    ...
                    HEAD_DIM;
                FLASHINFER_CUDA_CALL(gpuFuncSetAttribute(reinterpret_cast<const void *>(kernel),
                ...
                FLASHINFER_CUDA_CALL(
                    gpuOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, kernel, num_threads, smem_size));
                uint32_t max_num_kv_chunks =
                    (num_blocks_per_sm * num_sm) / (num_kv_heads * ceil_div(qo_len * group_size, num_rows_per_cta));
                uint32_t num_chunks;
                ...
                else
                {
                    ...
                    {
                        FLASHINFER_CUDA_CALL(
                            MergeStates(tmp, tmp_lse, o, lse, num_chunks, qo_len, num_qo_heads, HEAD_DIM, stream));
                    }
                    else
                    {
                        FLASHINFER_CUDA_CALL(AttentionSum(tmp, o, num_chunks, qo_len, num_qo_heads, HEAD_DIM, stream));
                    }
                }
            }
        })
...
template <MaskMode MASK_MODE, PosEncodingMode POS_ENCODING_MODE, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D,
          uint32_t NUM_MMA_KV, uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, typename DTypeQKAccum,
          typename AttentionVariant>
__global__ __launch_bounds__(NUM_WARPS_Q *NUM_WARPS_KV *WARP_SIZE) void BatchPrefillWithRaggedKVCacheKernel(
    const uint_fastdiv group_size,
#if defined(__HIPCC__) || (defined(__clang__) && defined(__HIP__)) || defined(__HIPCC_RTC__)
    const typename AttentionVariant::ParamsT params)
{
    ...
    const __grid_constant__ typename AttentionVariant::ParamsT params)
{
    ...
    else
    {
        ...
        const uint32_t qo_packed_idx_base = __builtin_amdgcn_readfirstlane(
        ...
        constexpr SwizzleMode swizzle_mode_q = SwizzleMode::k128B;
        smem_t<swizzle_mode_q> qo_smem(smem);
        uint32_t q_ptr_thread_offset = get_elem_offset_impl(q_indptr[request_idx], kv_head_idx * group_size,
        ...
        load_q_global_smem<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D>(
            qo_packed_idx_base, qo_upper_bound, q, q_ptr_thread_offset, q_stride_n, q_stride_h, group_size, &qo_smem);
        ...
        if constexpr (POS_ENCODING_MODE == PosEncodingMode::kRoPELlama)
        {
            ...
            {
                q_smem_inplace_apply_rotary_with_pos<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D, swizzle_mode_q,
                                                     DTypeQ>(qo_packed_idx_base, q_offset + q_indptr[request_idx],
                                                             &qo_smem, group_size, &q_smem_offset_r, rope_freq);
            }
            block.sync();
        }
...
#pragma unroll
        ...
        // write_back
        const uint32_t o_packed_idx_base = qo_tile_idx * NUM_WARPS_Q * NUM_MMA_Q * 16;
        write_o_reg_gmem<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D>(
            o_frag, &qo_smem, o_ptr_base, o_packed_idx_base, qo_len,
            /*o_stride_n=*/
...
template <uint32_t CTA_TILE_Q, uint32_t HEAD_DIM, PosEncodingMode POS_ENCODING_MODE, bool ALLOW_FP16_QK_REDUCTION,
          MaskMode MASK_MODE, typename AttentionVariant>
                                                   ...
                                                   gpuStream_t stream)
{
    ...
    FLASHINFER_CUDA_CALL(gpuGetDevice(&dev_id));
    int max_smem_per_sm = 0;
    FLASHINFER_CUDA_CALL(gpuDeviceGetAttribute(&max_smem_per_sm, gpuDevAttrMaxSharedMemoryPerMultiprocessor, dev_id));
    // we expect each sm execute two threadblocks
    // TODO(Zihao): fix the following computation
    const int num_ctas_per_sm = max_smem_per_sm > (16 * HEAD_DIM * sizeof(DTypeQ) * 16) ? 2 : 1;
    const int max_smem_per_threadblock = max_smem_per_sm / num_ctas_per_sm;

    const uint32_t max_num_mma_kv_reg = (HEAD_DIM >= 128 && NUM_MMA_Q == 2 &&
    ...
    DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem, max_num_mma_kv_reg), NUM_MMA_KV, {
        ...
        else
        {
            ...
            uint32_t smem_size =
                ...
                HEAD_DIM;
            auto kernel =
                BatchPrefillWithRaggedKVCacheKernel<MASK_MODE, POS_ENCODING_MODE, NUM_MMA_Q, NUM_MMA_D, NUM_MMA_KV,
                                                    NUM_WARPS_Q, NUM_WARPS_KV, DTypeQKAccum, AttentionVariant>;
            FLASHINFER_CUDA_CALL(gpuFuncSetAttribute(reinterpret_cast<const void *>(kernel),
                                                     gpuFuncAttributeMaxDynamicSharedMemorySize, smem_size));
            if (tmp_v == nullptr)
