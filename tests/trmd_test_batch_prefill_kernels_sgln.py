...
@pytest.mark.parametrize("batch_size,qo_len,kv_len,kv_mult", [
    (1, 6, 6, 3229565),
    (1, 796, 796, 3229565),
    (1, 864, 864, 3229565),
    (10, 912, 912, 3229565),
...
@pytest.mark.parametrize("num_qo_heads,num_kv_heads", [(6, 1)])
...
def test_batch_prefill_with_paged_kv_cache(
    ...
    #rand qo_len
    qo_len_seq = torch.randint(qo_len, size=(batch_size*2,)).int() + 1
    q_indptr_cpu = torch.zeros(size=(batch_size+1,)).int()
    accm = 0
    for i in range(1,batch_size+1):
        accm = accm + qo_len_seq[i-1]
        q_indptr_cpu[i] = accm
    q = torch.randn(accm, num_qo_heads, head_dim).to(dtype).to(0)
    #equal qo_len
    #q = (torch.randn(batch_size * qo_len, num_qo_heads, head_dim)*6.0).to(dtype).to(0)
    #q_indptr_cpu = torch.arange(0, batch_size + 1).int() * qo_len

    num_pages_per_seq = (kv_len + page_size - 1) // page_size
    total_num_pages = int((kv_mult/(kv_len*batch_size)) * num_pages_per_seq * batch_size)
    if kv_layout == "HND":
    ...
    if not contiguous_kv:
        ...
        for v in kv_shape[1:]:
            ...
            tmp.append(v)
        kv_shape = tmp
        kv_data_fp32 = (torch.randn(*kv_shape, dtype=torch.float32)*3.0).to(0)
